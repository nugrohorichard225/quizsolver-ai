...
const QuizCard: React.FC<QuizCardProps> = ({ question, index, onSelectOption, onCheckAnswer, disabled, quizMode }) => {
  ...
  return (
    <div 
      id={`question-${index}`} 
      className={`bg-white rounded-xl shadow-sm border p-6 transition-all duration-300 scroll-mt-24
    ...
    `}>
      ...
      <div className="space-y-3">
        {question.options.map((option, idx) => {
          ...
          let optionClass = "group relative p-4 rounded-lg border text-sm transition-all duration-200 flex justify-between items-center ";
          ...
          return (
            ...
            >
              <div className="flex items-center">
                 <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold mr-3 border transition-colors
      ...
      {isValidated && (
        <div className={`mt-6 p-5 rounded-lg border animate-in fade-in slide-in-from-top-2 duration-300
